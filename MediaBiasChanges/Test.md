...
permalink: /digital-famine/media-lit/test/
...
function buildLoginModal() {
    ...
    modal.innerHTML = `
        <div style="background:linear-gradient(135deg, #353e74ff, #9384d5ff);padding:40px;border-radius:20px;box-shadow:0 10px 40px rgba(0,0,0,0.3);text-align:center;max-width:420px;">
            <h2 style="color:#ffffff;margin-bottom:10px;font-size:1.6rem;">Login</h2>
            <p style="color:#c8d7eb;margin-bottom:18px;">Sign in with your account</p>
            
            <div style="text-align:left;margin-bottom:15px;">
                <label style="color:#e0e7ff;display:block;margin-bottom:5px;font-weight:600;">GitHub ID</label>
                <input type="text" id="login-uid" placeholder="student1" style="width:100%;padding:12px;border-radius:10px;border:2px solid #4299e1;font-size:1rem;background:rgba(136, 134, 172, 0.9);color:white;" />
            </div>
            
            <div style="text-align:left;margin-bottom:15px;">
                <label style="color:#e0e7ff;display:block;margin-bottom:5px;font-weight:600;">Password</label>
                <input type="password" id="login-password" placeholder="••••••••" style="width:100%;padding:12px;border-radius:10px;border:2px solid #4299e1;font-size:1rem;background:rgba(136, 134, 172, 0.9);color:white;" />
            </div>
            
            <div style="display:flex;gap:10px;">
                <button id="login-cancel-btn" style="flex:1;padding:12px;border-radius:10px;background:rgba(255,255,255,0.2);color:white;font-weight:700;border:none;cursor:pointer;">Cancel</button>
                <button id="login-submit-btn" style="flex:1;padding:12px;border-radius:10px;background:#4299e1;color:white;font-weight:700;border:none;cursor:pointer;">Login</button>
            </div>
    ...
    async function handleLogin() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                sessionStorage.setItem('playerName', uid);
                modal.remove();
...
window.signOut = function() {
    ...
    document.cookie = 'jwt_python_flask=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=None; Secure';
    window.location.reload();
};
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Performance Survey</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        #result-resources::-webkit-scrollbar {
            width: 8px;
        }

        #result-resources::-webkit-scrollbar-track {
            background: rgba(255,255,255,0.1);
            border-radius: 10px;
        }

        #result-resources::-webkit-scrollbar-thumb {
            background: rgba(255,255,255,0.3);
            border-radius: 10px;
        }
...
<body>
    <div class="performance-survey">
        <div class="survey-content">
            ...
            <div class="rating-buttons">
                ...
                <button class="rating-btn" onclick="submitRating(2)">2<br><span>Fair</span></button>
                <button class="rating-btn" onclick="submitRating(3)">3<br><span>Good</span></button>
                <button class="rating-btn" onclick="submitRating(4)">4<br><span>Excellent</span></button>
                <button class="rating-btn" onclick="submitRating(5)">5<br><span>Superior</span></button>
            </div>
        </div>
    </div>
    ...
    <div id="results-modal" class="modal">
        <div class="modal-content">
            ...
            <button class="close-btn" onclick="closeModal()">Got it, thanks!</button>
        </div>
    </div>
    ...
    <script>
        ...
        function showResults(data) {
            ...
            resources.innerHTML = `
                ...
                <p>${userResources.intro}</p>
                ${userResources.items.map(r => `<div>${r}</div>`).join('')}
            `;

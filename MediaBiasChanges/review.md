...
| Screenshot | Issue | Debugging Process | Solution | Evidence of Learning |
|---|---|---|---|---|
| <img width="622" height="221" alt="Terminal" src="https://github.com/user-attachments/assets/a0597162-3583-435f-a1eb-eed061f168d3" > | GET request to `/api/id` returns `401 UNAUTHORIZED` (shown in terminal logs) | Terminal shows multiple 401 errors when accessing endpoint. Flask dev server running on port 8001 | Need to investigate authentication middleware blocking the route | Server is responding but rejecting requests - indicates auth layer is active and blocking public access to endpoint |
| <img width="931" height="230" alt="Code" src="https://github.com/user-attachments/assets/ba39499f-50b6-44ca-abeb-93101c108c54" > | Route defined to return `{"id": "guest"}` with status 200, but comment suggests making it public | Code shows `@app.route('/api/id', methods=['GET', 'OPTIONS'])` with comment "Add this if you want it public" | Add authentication bypass decorator or modify `before_request` handler to allow public access | Route definition exists and is correct, but lacks decorator to bypass authentication middleware - explains why requests are blocked |
| <img width="440" height="131" alt="API Response" src="https://github.com/user-attachments/assets/006c166e-847a-4020-b1af-78f6062aa294" > | API returns `404 NOT FOUND` with error message "Thesis not found" instead of expected guest ID | Testing shows `{"success": false, "error": "Thesis not found"}` response in 4ms | Search codebase for "Thesis not found" string to find custom error handler intercepting requests | Discovered custom 404 error handler is overriding route response - need to either fix route registration order or modify error handler to exclude this endpoint |
